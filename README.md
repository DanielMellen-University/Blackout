...
Production builds split the Three.js runtime into a cacheable vendor chunk, keeping the game entry smaller without changing flight or render behavior.
...
- The live HUD now shows progress toward the next altitude reward tier, while the milestone payout remains gated by the existing completed-run handoff.
- Terrain worker results now drain through a nearest-first pop path, preserving stream order while avoiding repeated ready-queue array shifts during fast movement.
...
- Nearby traffic now appears as low-priority `TRAFFIC` radar contacts without stealing city or village destination locks.
- Nearby airborne traffic now gives a one-shot `TRAFFIC LEFT`, `RIGHT`, `AHEAD`, or `BEHIND` range cue, with no collision state or continuous audio cost.
- The radar now includes a compact six-marker scope with bounded distance projection and distinct gate, settlement, and traffic colors, while keeping the cached text readout for accessibility.
- Traffic contacts now carry signed altitude separation, so proximity alerts and radar markers distinguish `ABOVE`, `BELOW`, and `LEVEL` without adding collision simulation or scene objects.
...
- Curated course cards retain the style attached to each course's best score, turning a debrief identity into a repeatable mastery target.
...
- Played course cards now show the next mastery tier target, keeping long-term route progression readable before launch without render-loop or storage work.
- The title progress line now includes how many curated courses have reached Legend mastery, alongside completion and badge totals.
...
- Crossing a career-rank threshold now reuses the milestone cue and marks the debrief with `CAREER ... UP`, keeping promotion feedback bounded and event-driven.
...
- Course cards now preview saved sortie streaks and contract wins, keeping repeat-run progression visible alongside individual flight records.
...
- `BUTTER LANDING` now shows the existing pre-landing quality forecast as cached task progress, while the actual touchdown still decides completion.
...
- A compact RISK row keeps generated route difficulty and rhythm visible during flight, so technical, tempo, and altitude lines remain readable after the briefing.
...
- Bonus contracts can include `BURN RUN`, rewarding controlled high-speed afterburner time while reusing the existing fuel and heat safeguards.
- The live TASK row keeps each contract's instruction visible during flight, so the target remains readable after transient briefing banners fade.
...
- Bonus contracts can also include `PRECISION CHAIN`, rewarding three consecutive high-quality gate clears while a rough pass resets only that chain.
- Bonus contracts can also include `NIGHT FLIGHT`, rewarding a short airborne stretch through the existing dusk and night envelope.
...
- Terrain worker slope shading now fills a caller-owned color tuple, preserving the same relief colors without allocating a result and rock-tint tuple per vertex.
