...
import {
  ...
  type StormBuffetOffset,
} from '../systems/StormBuffet'
...
export class CameraSystem {
  ...
  setStormBuffet(intensity: number): void {
    ...
    this.stormTarget = MathUtils.clamp(intensity, 0, 1)
  }
  ...
  private applyStormBuffet(dt: number, cockpit: boolean): void {
    ...
    const viewScale = stormBuffetViewScale(cockpit, this.renderQuality === 'low')
    const target = this.stormTarget * viewScale
    this.stormSway = dt <= 0
      ? target
      : MathUtils.damp(this.stormSway, target, 5.5, dt)
    if (this.stormSway <= 0.001) {
      this.stormSway = 0
      return
    }
    this.stormPhase = (this.stormPhase + Math.max(dt, 0.008) * (9 + this.stormSway * 7)) % (Math.PI * 2)
    const offset = stormBuffetOffsetInto(this.stormOffset, this.stormPhase, this.stormSway)
    this.camera.position.x += offset.x
    this.camera.position.y += offset.y
    this.camera.position.z += offset.z
  }
