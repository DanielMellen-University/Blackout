...
export function sampleGeography(x: number, z: number): Climate {
  ...
  const range = Math.pow(ridged(wx / 7500 + 52, wz / 7500 - 13, 3), 1.5)
    * highlands * inland
  const crest = Math.pow(ridged(wx / 1700, wz / 1700, 3), 1.5)
  const rolling = (fbm(wx / 1350, wz / 1350, 3) - .4) * 85
  ...
  height += range * (650 + crest * 2400)
  const dunePhase = wx / 95 + Math.sin(wz / 700) * 1.4 + fbm(wx / 1100, wz / 1100, 2) * 5
  const duneHeight = Math.pow((Math.sin(dunePhase) + 1) * .5, 2.4) * 34
  ...
  if (shore < 0) {
    height = shore * 2600 - 4
    height += ridged(wx / 2200, wz / 2200, 2) * 18 * smoothstep(-.14, -.02, shore)
  ...
  if (shore > 0) {
    ...
    if (distance < width + 1) waterLevel = level
    ...
    if (gate > 0) {
      if (kind < .5) {
        const level = 45 + hash2(cx + 15, cz - 62) * 190
        ...
        if (r < 1.1 && enabled * inland > .999) waterLevel = level
      } else if (kind < .77) {
        const r = distanceToLandmark / size
        volcanic = gate
        const cone = Math.pow(Math.max(0, 1 - r / 2.2), 1.4) * 1450
        const crater = (1 - smoothstep(.18, .48, r)) * 510
  ...
  let biomeB: Biome
  let biomeMix = 0
  const snowLine = 1100 - cold * 500 + hot * 350
  if (height < waterLevel) {
    biome = waterLevel <= 0 ? 'ocean' : 'water'
    biomeB = biome
  } else {
    const alpine = smoothstep(250, 800, height)
    const snow = smoothstep(snowLine - 100, snowLine + 240, height)
    const low = 1 - alpine
  ...
  return {
    height, waterLevel, moisture, temperature, biome, biomeB, biomeMix, land,
