...
function routeOffsets(
  ...
  if (profile === 'canyon') {
    const weave = 270 + seedPhase * 80
    return [
      { forward: 560, right: 0, height: 96 },
      { forward: 1_040, right: weave, height: 112 },
  ...
  if (profile === 'river') {
    const bend = 360 + seedPhase * 110
    return [
      { forward: 620, right: 0, height: 72 },
      { forward: 1_180, right: bend, height: 88 },
